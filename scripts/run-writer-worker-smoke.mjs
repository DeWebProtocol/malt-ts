...
try {
  ...
  const next = JSON.parse(await writer.applyAuthentication(backend, bytes(base.handle), json({
    profile: 'malt.authentication-delta/1', changes: [{ label: 'ZmlsZQ==', after: 'bafkqaaa' }]
  })));
  ...
  const batch = { profile: 'malt.authentication-batch/1', transaction_id: 'worker-smoke', base: base.root, root: next.root, candidates: [exported] };
  const digest = await writer.validateAuthenticationBatch(backend, json(batch));
  const receipt = { profile: 'malt.authentication-receipt/1', transaction_id: batch.transaction_id, base: batch.base, root: batch.root, digest, durable_boundary: 'worker-smoke/0' };
